...
//                     "city":city, "address": address, "buyers": userId, "book": book_list},{
//                         headers:{
//                             "Authorization": token
//                         }
//                 }
...
const ModalAdd = ({ active, setActive, data, price }) => {
  ...
  const postRequest = () => {
    if (data.length !== 0 && address && city) {
      const bookList = data.map((value) => value.Book_name);
      const bookStr = bookList.join(', ');
      axios.post(
        'http://127.0.0.1:8000/api/Order/',
        {
          "city": city,
      ...
      <div className={active ? "modal active" : "modal"} onClick={() => setActive(false)}>
        <div className="modal_content" onClick={e => e.stopPropagation()}>
          ...
          <form className="input-data3" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="cta2" onClick={postRequest}>
              {isLoading ? <span>Loading...</span> : <span>Тапсырыс беру</span>}
            </button>
          </form>
